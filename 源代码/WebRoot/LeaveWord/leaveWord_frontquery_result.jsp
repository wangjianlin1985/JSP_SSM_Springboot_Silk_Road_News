		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" class="active"><a href="#leaveWordListPanel" aria-controls="leaveWordListPanel" role="tab" data-toggle="tab">留言建议列表</a></li>
			    	<li role="presentation" ><a href="<%=basePath %>LeaveWord/leaveWord_userFrontAdd.jsp">添加留言建议</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>留言标题</td><td>留言时间</td><td>留言人</td><td>回复时间</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<leaveWordList.size();i++) {
					    	            		...
					    	            		LeaveWord leaveWord = leaveWordList.get(i); //获取到留言建议对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											...
 											<td>
 												<a href="<%=basePath  %>LeaveWord/<%=leaveWord.getLeaveId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="leaveWordEdit('<%=leaveWord.getLeaveId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="leaveWordDelete('<%=leaveWord.getLeaveId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;留言建议信息编辑</h4>
      </div>
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="leaveWord_leaveId_edit" class="col-md-3 text-right">留言id:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="leaveWord_leaveId_edit" name="leaveWord.leaveId" class="form-control" placeholder="请输入留言id" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
...
/*弹出修改留言建议界面并初始化数据*/
function leaveWordEdit(leaveId) {
	$.ajax({
		url :  basePath + "LeaveWord/" + leaveId + "/update",
		type : "get",
		dataType: "json",
		success : function (leaveWord, response, status) {
			if (leaveWord) {
				$("#leaveWord_leaveId_edit").val(leaveWord.leaveId);
...
/*删除留言建议信息*/
function leaveWordDelete(leaveId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
...
/*ajax方式提交留言建议信息表单给服务器端修改*/
function ajaxLeaveWordModify() {
	$.ajax({
		url :  basePath + "LeaveWord/" + $("#leaveWord_leaveId_edit").val() + "/update",
		type : "post",
		dataType: "json",
